...
#ifndef __WAVESMVMTP_H
#define __WAVESMVMTP_H
...
class MV_ColMat
{
	...
	public:
		...
		MV_ColMat(TYPE*, unsigned int m, unsigned int n, unsigned int lda, Matrix_::ref_type i);
...
MV_ColMat<TYPE>::MV_ColMat() :
		v_(), dim0_(0), dim1_(0), lda_(0), ref_(0)
{
}

template<class TYPE>
MV_ColMat<TYPE>::MV_ColMat(unsigned int m, unsigned int n) :
		v_(m * n), dim0_(m), dim1_(n), lda_(m), ref_(0)
{
}

template<class TYPE>
MV_ColMat<TYPE>::MV_ColMat(unsigned int m, unsigned int n, const TYPE &s) :
		v_(m * n), dim0_(m), dim1_(n), lda_(m), ref_(0)
{
	operator=(s);
}
...
template<class TYPE>
inline const TYPE& MV_ColMat<TYPE>::operator()(unsigned int i, unsigned int j) const
{
#ifdef MV_MATRIX_BOUNDS_CHECK
	assert(0<=i && i<size(0));
	assert(0<=j && j<size(1));
...
MV_ColMat<TYPE>::MV_ColMat(const MV_ColMat<TYPE> & m) :
		v_(m.dim0_ * m.dim1_), dim0_(m.dim0_), dim1_(m.dim1_), ref_(0), lda_(m.dim0_)
{
...
template<class TYPE>
inline MV_ColMat<TYPE>::MV_ColMat(TYPE* d, unsigned int m, unsigned int n, Matrix_::ref_type i) :
		v_(d, m * n, MV_Vector_::ref), dim0_(m), dim1_(n), lda_(m), ref_(i)
{
}

template<class TYPE>
inline MV_ColMat<TYPE>::MV_ColMat(TYPE* d, unsigned int m, unsigned int n, unsigned int lda, Matrix_::ref_type i) :
...
template<class TYPE>
MV_ColMat<TYPE>::MV_ColMat(TYPE* d, unsigned int m, unsigned int n, unsigned int lda) :
...
MV_ColMat<TYPE> MV_ColMat<TYPE>::operator()(const MV_VecIndex &I, const MV_VecIndex &J)
{
	...
	{
		cerr << "Matrix index: (" << I.start() << ":" << I.end() << "," << J.start() << ":" << J.end() << ") not a subset of (0:" << dim0_ - 1 << ", 0:" << dim1_ - 1 << ") " << endl;
		exit(1);
	}

	// this automatically returns a reference
	// 
	return MV_ColMat<TYPE>(&v_[J.start() * lda_ + I.start()], I.end() - I.start() + 1, J.end() - J.start() + 1, lda_, Matrix_::ref);
}

template<class TYPE>
const MV_ColMat<TYPE> MV_ColMat<TYPE>::operator()(const MV_VecIndex &I, const MV_VecIndex &J) const
{
	...
	return MV_ColMat<TYPE>(&(t->v_[J.start() * lda_ + I.start()]), I.end() - I.start() + 1, J.end() - J.start() + 1, lda_, Matrix_::ref);
}
