...
// compute the right hand side vector of equation for q using the scattered wave:
...
void ComputeRHS_q_totalwave(double A3n, double A4n, Vec_real Sum_dqdx, Vec_real Sum_dqdy, Vec_real Sum_dqdz, 
		      Vec_real dVdx, Vec_real dVdy, Vec_real dVdz, Vec_real dqdx, Vec_real dqdy, Vec_real dqdz, Vec_real& RHS_vec)
{
	int NrFEMNodes = RHS_vec.size(); 
	int i;
...
// Compute the coefficient matrix for the equation for q using the scattered wave:
...
// Compute the function w = wi*exp(s^2v) using the scattered wave:
...
void Compute_w_from_v_totalwave(double s, Vec_real v, Vec_real& w)
{
...
void Compute_tail_from_w_totalwave(double s, Vec_real w, Vec_real& V)
{
...
// == step 0.1: load the measured data, calculate the boundary values: 
	...
	remove("Test_L2Coeff.m"); remove("Test_MaxCoeff.m"); remove("Test_RelativeError.m"); remove("Test_L2Error.m");
...
{	for (int n=0; n < Ns; n++) //loop w.r.t. pseudo frequencies
	{	STOP = false;
		...
		{	V_old = vector_copy(V); // for stopping only!
			...
			//compute_coefficient_FDM(Coefficient, v, sdg_FEM, pseudoFreq(n+1)); // using the FDM
			
			smooth_coefficient_FDM(Coefficient,sdg_FEM); // coefficient averaging
			...
			cout << " Iteration " << Iter << ", Relative L2 error =  " << RelativeError(Iter) 
			     << ", L2-error =  " << L2Error(Iter) << endl;
			cout << " Max Coeff = " << MaxCoeff(Iter) << ", L2-norm coeff =  " << L2Coeff(Iter) << endl; 
