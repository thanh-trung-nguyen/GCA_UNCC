/* 
Function gca_totalwave.C solves the coefficient identification problem using the globally convergent algorithm
input: 
...
void coefficient_truncate(Vec_real& Coefficient, double C_lb, double C_ub, WavesGridB& gg, Vec_real Subdomain)
{
	...
	{	
		...
		{	if (Coefficient(i) < C_lb)
				Coefficient(i) = C_lb; // background value
...
void coefficient_truncate(Vec_real& Coefficient, double C_lb, double C_ub, WavesGridB& gg, Vec_real Subdomain, Mat_real CoeffMask)
{
	...
	xmin = gg.getCoor(0,1);
 	ymin = gg.getCoor(0,1);
	xmax = gg.getCoor(NrFEMNodes-1,0);
	ymax = gg.getCoor(NrFEMNodes-1,1);

	hx = (xmax - xmin)/(CoeffMask.size(0)-1);
	hy = (ymax - ymin)/(CoeffMask.size(1)-1);
//cout << xmin << " " << xmax << ", " << ymin << " " << ymax << ": " << hx << " " << hy << endl;
	for (int i=0; i< NrFEMNodes; i++)
	{		 
		double x = gg.getCoor(i,0); 
		double y = gg.getCoor(i,1);
		double z = gg.getCoor(i,2); 
		ix = round((x- xmin)/hx); iy = round((y - ymin)/hy); // cout << ix << " " << iy << endl;
...
int main(int argc, char **argv)
{
	...
	{	Coefficient = C_ub; cout << "Upper bound: " << Maxim(Coefficient) << endl; 
	...
	for (i=0; i < Nxy; i++)
		meagaussian(i) = fabs(bdc_tail(NrFEMNodes-Nxy+i)); // absolute values of measured function v for least squares error. 
...
// ++++++++++ MAIN LOOP:
//============================================================================================================
for (int ii =0; ii < 1; ii++) // repeating the whole process: not now!
{	for (int n=0; n < Ns; n++) //loop w.r.t. pseudo frequencies
	{	STOP = false;
		...
		{	//cout << i << " " << BoundaryNodes(i)-1 << " " << psi(n,i) << endl; 
		...
		{	V_old = V; // for stopping only!
			Coeff_old = Coefficient; // for stopping only!

			V2 = V;
			//if (firsttail.compare("exact") != 0)	
			//if (Iter > 0)
			//    V2 = vector_multiplication(V2,-1.0/s_max);	// test using the derivative of tail
			...
			// test: simulated function v at the back scattering side
			for (i=0; i < Nxy; i++)
    				simgau(i) = V(NrFEMNodes-Nxy+i); //log(w(NrFEMNodes-Nxy+i)/Wi(NrFEMNodes-Nxy+i))/s(0)/s(0); 
			...
			double MaxGauss = Maxim(meagaussian);
//			L2Error(Iter) = fabs(MinGauss - Minim(simgau)); 
			for (i = 0; i < Nxy; i++)
			    { if (meagaussian(i) >= 0.9*MaxGauss)
				L2Error(Iter) = L2Error(Iter) + pow(fabs(simgau(i)) - meagaussian(i),2); //test comparing the measured data with the simulated one
			    }	



			RelativeError(Iter) = L2_norm(gg, vector_subtraction(V,V_old));
			...
			cout << " Iteration " << Iter << ", Max Coeff = " << MaxCoeff(Iter) << ", Min Coeff = " << Minim(Coefficient) <<   ", Relative error =  " << RelativeError(Iter) << ", L2-error =  " << L2Error(Iter) << endl;

			if ((Iter > 0) && (RelativeError(Iter) >= 1*RelativeError(Iter-1)))
				STOP = true; 
			if (Iter >= MaxIter(n)-1 || L2Error(Iter) <=1e-8 || RelativeError(Iter) < 1e-8)
				STOP = true;
			Iter++; 
